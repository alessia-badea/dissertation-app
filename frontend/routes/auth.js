...
const authController = require('../../backend/config/controllers/authController');
const jwt = require('jsonwebtoken');
const bcrypt = require('bcrypt');
const User = require('../../backend/models/User');
const { requireAuth, requireRole } = require('../../backend/middleware/auth'); // <= ADĂUGAT
...
router.post('/logout', authController.logout);      // <= ADĂUGAT
router.get('/me', authController.getCurrentUser);   // <= ADĂUGAT

// Exemple cu middleware (Backend Dev 2)
router.get('/profile', requireAuth, (req, res) => {          // <= ADĂUGAT
  res.json({ user: req.session.user });
});

router.get('/professor-only', requireRole('professor'), (req, res) => { // <= ADĂUGAT
...
router.post('/register-jwt', async (req, res) => { // <= redenumit ca să nu calce peste /register
  try {
...
router.post('/login-jwt', async (req, res) => { // redenumit ca să nu calce peste /login
  try {
